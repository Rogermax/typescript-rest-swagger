...
export class SpecGenerator {
  private readonly debugger = debug('typescript-rest-swagger:spec-generator')
  ...
  private buildDefinitions () {
    ...
    Object.keys(this.metadata.referenceTypes).map((typeName) => {
      ...
      if (referenceType.additionalProperties) {
        definitions[referenceType.typeName].additionalProperties =
  ...
  private getSwaggerTypeForEnumType (enumType: EnumerateType): Swagger.Schema {
    function getDerivedTypeFromValues (values: any[]): string {
    ...
    return {
      enum: enumType.enumMembers.map((member) => member) as [string],
